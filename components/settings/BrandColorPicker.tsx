  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center gap-3">
        <label className="relative h-11 w-11 shrink-0 cursor-pointer overflow-hidden rounded-stripe-sm border border-stripe-hairline-input">
          <input
            type="color"
            value={isValidHex ? value : '#1c69d4'}
            onChange={(event) => onChange(event.target.value)}
            aria-label="Márkaszín kiválasztása színválasztóval"
            className="absolute -left-1 -top-1 h-[calc(100%+8px)] w-[calc(100%+8px)] cursor-pointer border-0 p-0"
          />
        </label>
        ...
        <input
          ...
          className={`tabular-nums-stripe h-11 w-36 rounded-stripe-sm border bg-white px-3 font-sohne text-[15px] text-stripe-ink placeholder:text-stripe-ink-mute focus:outline-none focus:ring-2 focus:ring-stripe-primary/30 focus:border-stripe-primary ${
