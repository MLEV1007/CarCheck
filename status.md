...
_Utolsó frissítés: 2026-07-31 (Márka/típus dropdownok, szigorú validáció, Általános autó fotók modul)_
...
- A `/inspections/[id]` szerkesztő/részletező oldalt tartalmazó commit (`d475379`) és a márka/típus dropdown + validáció + Általános autó fotók modul commitja lokálisan megvannak, de **NINCSENEK pusholva** -- lásd a "Következő lépés" pontnál a részleteket.
...
## Ellenőrzés
- `npx tsc --noEmit` — hibamentes (legutóbb ellenőrizve a márka/típus dropdown + validáció + Általános autó fotók modul elkészülte után).
- Supabase security advisor (`get_advisors`, `nsejmkcwvksbwxscvrvb` projekt) — újrafuttatva a `general_photos` migráció + RPC-bővítés után, nincs új figyelmeztetés (a 3 ismert WARN változatlan, lásd lent).
- Adatbázis séma élesben ellenőrizve (`list_tables`, `nsejmkcwvksbwxscvrvb`): az `inspections.general_photos` oszlop (`text[] not null default '{}'`) sikeresen létrejött, RLS-t nem igényelt (a meglévő `inspections`-szintű policy-k lefedik).
...
## Következő lépés
- **Push a lokális commit(ok)nak:** a `/inspections/[id]` oldal (`d475379`) és a márka/típus dropdown + validáció + Általános autó fotók modul commitja csak lokálisan létezik a felhasználó gépén (a sandbox nem tud hitelesíteni a GitHub felé) -- fusd le `git push`-t a saját terminálodból/Git klienseddel a `CarCheck` mappában, hogy a `main` ág naprakész legyen.
- Kézi funkcionális teszt élesben: márka/típus dropdown váltogatása + "Egyéb/Más" mindkét szinten, évjárat/km óra állás/VIN/rendszám hibás értékekkel (túl korai évjárat, irreális km, 17+ karakteres VIN), több általános fotó egyszerre feltöltése + törlése + piszkozat-szerkesztésnél nem duplikálódnak-e, publikus riport "Gépjármű fotók" galéria + lightbox.
- Piszkozat folytatása meglévő méréssel/hibával (média nem töltődik fel újra feleslegesen), publikálás utáni "Visszaállítás piszkozatba" -> a publikus link tényleg 404-et mutat-e utána, majd újra-publikálás után helyreáll-e.
