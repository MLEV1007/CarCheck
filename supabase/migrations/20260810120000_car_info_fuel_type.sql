-- Autó alapadatok kiegészítése: Üzemanyag típusa (2026-08-10, felhasználói kérés --
-- "A forgalmiból kell az is, hogy milyen üzemanyagú az autó. Elektromos, benzin vagy
-- dízel" -- lásd status.md).
--
-- ZÁRT, 3-elemű választás (nem szabad szöveges, ellentétben az `engine_type`-tal) --
-- a wizard "Autó adatok" (1.) lépésén (AI-fotószkennerrel VAGY kézzel) rögzítve, lásd
-- `components/inspections/wizard/StepCarInfo.tsx` és `app/api/ai/scan-vin/route.ts`.
-- Ugyanaz a minta, mint a `20260809180000_car_info_engine_power_weight.sql` migráció.
--
-- Nincs szükség új RLS policy-ra -- az új oszlop a MEGLÉVŐ `inspections` sor részeként
-- öröklődik, a meglévő `inspections_select_own`/`inspections_select_org`/insert/update/
-- delete policy-k `user_id`/`organization_id` alapú szűrése változatlanul érvényes rá.

alter table public.inspections
  add column fuel_type text
    check (fuel_type is null or fuel_type in ('benzin', 'dizel', 'elektromos'));

comment on column public.inspections.fuel_type is
  'Üzemanyag típusa a forgalmi engedély P.3 mezőjéről -- ZÁRT enum (''benzin''/''dizel''/''elektromos''), opcionális.';

-- `get_public_report` RPC frissítése -- a `fuel_type` oszlopot a `jsonb_build_object(''inspection'',
-- ...)` blokkba, közvetlenül a `gross_weight_kg` UTÁN vesszük fel, hogy a publikus riporton
-- (`ReportHero.tsx`) és a riport AI chatben (`app/api/report-chat/route.ts`) is megjelenjen --
-- lásd a `20260809180000_car_info_engine_power_weight.sql` migráció azonos elvű kommentjét.
-- A függvény törzse egyébként VÁLTOZATLAN.
...
begin
  ...
  select jsonb_build_object(
    'inspection', jsonb_build_object(
      ...
      'fuel_type', v_inspection.fuel_type,
